...
void handle_files_request(int socket_fd, struct http_request *request) {
    ...
    if (start_with(request->path, "/filter") &&
        strcmp(http_get_mime_type(request->path), "image/bmp") == 0) {
...
void dispatch(int client_socket_number) {
    ...
    if (request == NULL || request->path[0] != '/') {
        http_make_error(client_socket_number, 400);
        return;
    }

    if (strstr(request->path, "..") != NULL) {
        http_make_error(client_socket_number, 403);
        return;
    }
    ...
    *socket_number = socket(PF_INET, SOCK_STREAM, 0);
    ...
    while (1) {
        ...
        if (child_id == 0) {
            ...
            // 1) when there was an error in prctl, 2) when the parent has been
            // killed
