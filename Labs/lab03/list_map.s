...
create_default_list:
    addi sp, sp, -12 # prelogue
    sw  ra, 0(sp)
    sw  s0, 4(sp)
    sw  s1, 8(sp)
    li  s0, 0       # pointer to the last node we handled
    li  s1, 0       # number of nodes handled      s1=i
loop:   #do...
    li  a0, 8 #                                                a0 = 8
    jal ra, malloc      # get memory for the next node:        a1 = 8, a0 = 一块8字节地址
    sw  s1, 0(a0)   # node->value = i                          s1 存到 a0里
    sw  s0, 4(a0)   # node->next = last                        s0 存到 a0+4里
    add s0, a0, x0  # last = node                              s0 = a0(存到s0，下一轮s0会被存到下一个节点的a0+4（next））
    addi    s1, s1, 1   # i++                                  s1++
    addi t0, x0, 10     #                                      t0 = 10
...
malloc:
    addi    a1, a0, 0
    addi    a0, x0, 9
    ecall           # a1 = 8，a0 = 9 -> 9号指令：申请内存。ecall会自动申请8个字节的地址放到a0里
    jr  ra